...
import { SendIcon, SchoolIcon, FriendlyBotIcon } from './components/IconComponents';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (messageText: string) => {
    ...
    try {
      ...
      setMessages(prev => {
        const newMessages = [...prev];
        // Replace the placeholder or add the error message
  ...
  return (
    <div className="flex flex-col h-screen bg-yellow-50">
      <header className="bg-yellow-100 border-b border-yellow-200 p-4 shadow-sm">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
           ...
           <p className="text-xs text-center text-slate-500 mt-2 px-4">
             ¡Recordad que con paciencia y apoyo, el aprendizaje es una gran aventura!
           </p>
        </div>
      </footer>
    </div>
